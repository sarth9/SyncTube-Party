...
import type { PlayState, VideoState } from "../types";
...
interface YouTubePlayerBoxProps {
  ...
  canControl: boolean;
  onNativePlay: (currentTime: number) => void;
  onNativePause: (currentTime: number) => void;
  onNativeSeek: (currentTime: number, playState: PlayState) => void;
}
...
function getSafeTime(time: unknown): number {
...
function getStateTimeWithDrift(videoState: VideoState): number {
...
export function YouTubePlayerBox({
  videoState,
  canControl,
  onNativePlay,
  onNativePause,
  onNativeSeek,
}: YouTubePlayerBoxProps) {
  ...
  const pendingPauseTimerRef = useRef<number | null>(null);
  ...
  function applyRemoteState(state: VideoState, forceVideoReload = false) {
    ...
    loadYouTubeIframeApi().then(() => {
      ...
      const player = new window.YT.Player(containerRef.current, {
        ...
        events: {
          ...
          onError: () => {
            setPlayerError(
              "This YouTube video cannot be embedded. Please choose another video below."
            );
          },
        },
      });
  ...
  useEffect(() => {
    const intervalId = window.setInterval(() => {
      ...
      const previousObservedAt =
        lastObservedAtRef.current > 0 ? lastObservedAtRef.current : now;
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-white/70 bg-black shadow-soft">
      <div className="relative aspect-video w-full">
        <div ref={containerRef} className="h-full w-full" />

        {!isReady && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-slate-950 px-6 text-center text-white">
            <div className="mb-3 h-10 w-10 animate-pulse rounded-full bg-white/20" />
            <p className="text-lg font-bold">Loading YouTube Player...</p>
            <p className="mt-2 max-w-md text-sm text-slate-300">
              The player uses the official YouTube IFrame API.
            </p>
          </div>
        )}

        {playerError && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-slate-950 px-6 text-center text-white">
            <p className="text-lg font-bold">Video cannot be loaded</p>
            <p className="mt-2 max-w-md text-sm text-slate-300">
              {playerError}
            </p>
          </div>
        ...
        {!canControl && isReady && !playerError && (
          <div className="pointer-events-auto absolute inset-0 z-20 flex items-start justify-end bg-transparent p-4">
            <span className="rounded-full border border-white/20 bg-black/60 px-3 py-1 text-xs font-bold text-white backdrop-blur">
              Watch only
            </span>
          </div>
        )}
